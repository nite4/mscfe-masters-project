    ...
    try:
        ...
        for cs in crypto_symbols:
            ...
            klines = client.get_historical_klines(
                cs, Client.KLINE_INTERVAL_5MINUTE, "5 Oct, 2024", "8 Jan, 2025"
            )
...
def process_pairs_series(
    seriesX, seriesY, dfX=None, dfY=None, df=None, data_type="Close"
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if (dfX is not None) & (dfY is not None):
            priceX = (
                pd.DataFrame(
                    dfX[dfX["Symbol"] == seriesX][[data_type, "OpenTime"]]
                ).set_index("OpenTime")
            ...
            priceY = (
                pd.DataFrame(
                    dfY[dfY["Symbol"] == seriesY][[data_type, "OpenTime"]]
                ).set_index("OpenTime")
        ...
        # Using df of all data
            ...
            priceX = (
                pd.DataFrame(
                    df[df["Symbol"] == seriesX][[data_type, "OpenTime"]]
                ).set_index("OpenTime")
                #   .rename(index=seriesX)
            )
            t00 = priceX.index[0]
            priceY = (
                pd.DataFrame(
                    df[df["Symbol"] == seriesY][[data_type, "OpenTime"]]
                ).set_index("OpenTime")
                #   .rename(index=seriesY)
            )
            t10 = priceY.index[0]
        ...
        merged = pd.merge(left=priceX, right=priceY, left_index=True, right_index=True)
...
def run_cointegration_test(price_pairs, print_stats=False, plotting=False, std=2):
    ...
    try:
        ...
        if plotting:
            ...
            axes[0].set_title(
                f"Spread and Trading Thresholds of {tickerX} and {tickerY}"
            )
            ...
            axes[1].axhline(
                -std, color="green", linestyle="--", label="Lower Threshold"
            )
            ...
            axes[2].plot(
                price_normalized[tickerX],
                label=f"{tickerX} (Normalized)",
                linestyle="-",
            )
            axes[2].plot(
                price_normalized[tickerY],
                label=f"{tickerY} (Normalized)",
                linestyle="--",
            )
            axes[2].set_title(
                f"Price Correlation of {tickerX} and {tickerY} (Normalized)"
            )
        ...
        print(
            f"Failed to conduct stationarity test on {tickerX} and {tickerY}: {str(e)}"
        )
        log.error(
            f"Failed to conduct stationarity test on {tickerX} and {tickerY}: {str(e)}"
        )
...
def plot_spread(df, X, Y, s: float = 2.0, ax=None):
    ...
    try:
        ...
        ax.scatter(
            df[df["NormalizedSpread"] > s]["NormalizedSpread"].index,
        ...
        ax.scatter(
            df[df["NormalizedSpread"] < -1 * s]["NormalizedSpread"].index,
        ...
        plt.legend(fontsize=8, bbox_to_anchor=(1.3, 1), loc="upper right")
        plt.tight_layout()
